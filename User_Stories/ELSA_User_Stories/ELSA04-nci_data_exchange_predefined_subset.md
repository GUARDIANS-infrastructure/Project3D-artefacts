# ELSA Story 04: NCI Data Exchange of Predefined Subset
...
As a data steward, I want to share a predefined subset of a dataset stored on NCI with an authorised researcher, so that they can download data onto their authorized storage.
...
- Dataset is organised as pre-defined subset
- Target subset is specified in the data access request.
  - Example: MGRB dataset: ASPREE vs 45andUp
  - Example: MGRB dataset: ASPREE for commercial use vs ASPREE for non-commercial use
...
- [ ] If the researcher is authorized to download the subset directly:
  - [ ] the data steward can programmatically grant the researcher access to the target subset stored on NCI using Elsa.
...
- [ ] If the researcher is authorized to transfer the data in a dedicated storage only:
  - [ ] the data steward can programmatically initiate the transfer of the target subset stored on NCI using Elsa to the dedicated storage.
