...
- The event history contain useful information and is presented in an easy-to-understand manner (with interpretation of events and attribution to the name of the person responsible e.g. "[Name] Handler redacted attachements, New Attachments replace the redacted attachments." with the ability to access the different documents; or "[Name] Decider added a remark"). Individual events can be hidden or maximised. This is the most useful view of the three options. However it is hard to search within events, or link these events to the other sources (API or downloadable report).
